...
interface ProjectOverviewContentProps {
	...
	/** Show the back-navigation button. Defaults to true. */
	showBackButton?: boolean;
	/** Show the "Go to Kanban Board" navigation button. Defaults to true. */
...
export function ProjectOverviewContent({
	...
	const filteredTasks = tasks.filter((t) => {
		...
		return (
			!suggestedStageIds.includes(t.projectStage) &&
			t.projectStage !== archiveStageId
		);
	});
	...
	const isCampaignReportApproved =
		!isDigitalMarketing ||
		...
		if (
			...
			if (!isCampaignReportApproved) {
				toast({
					...
					description:
						"A campaign report must be uploaded and approved before completing a Digital Marketing project.",
					variant: "destructive",
				});
				return;
			}
	...
	const handleGracePeriodSave = async (
		expiresAt: string,
		notes: string
	) => {
		try {
	...
	const handleProvisionalPOSave = async (
		poNumber: string,
		expiresAt: string
	) => {
		try {
	...
	const gracePeriodExpired =
		hasGracePeriod &&
		new Date(project.gracePeriodExpiresAt!) < new Date();
	const hasProvisionalPO = !!project.provisionalPoNumber;
	const provisionalPOExpired =
		hasProvisionalPO && project.provisionalPoExpiresAt
				...
				<div className="flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800 p-4">
					...
					<div className="flex-1">
						...
						<p className="text-sm text-amber-700 dark:text-amber-400 mt-0.5">
							This project does not have a PO on file. Task creation
							is restricted until a PO is received.
						</p>
					</div>
					{canManageFinance && (
					...
					<CardContent className="flex-1 flex flex-col justify-center">
						{canChangeStatus ? (
							<div className="flex flex-col gap-1">
										...
										onChange={async (e) => {
											...
											try {
												...
												toast({
													...
													description:
														"Project deadline has been updated successfully.",
												});
											} catch (error) {
												toast({
													title: "Error",
								...
								{!project.deadline && !isEditingDeadline && (
				...
				<div className="lg:col-span-2 space-y-8">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Custom stages defined for this project's Kanban board
							</CardDescription>
						</CardHeader>
						<CardContent>
