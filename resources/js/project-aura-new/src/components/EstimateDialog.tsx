									...
									<PopoverContent className="w-[300px] p-0" align="start">
										<Command>
											...
											{formData.client_id > 0 && (
												<>
													<CommandGroup heading={`${clients.find(c => c.id === formData.client_id)?.company_name || 'Client'} Projects`}>
														{projects
															?.filter(p => p.clientId === formData.client_id)
															.map(p => (
																<CommandItem
																	key={p.id}
																	value={`${p.name}-${p.id}`}
													...
													{projects?.some(p => p.clientId !== formData.client_id) && (
														<CommandSeparator />
													)}
												</>
											)}
											<CommandGroup heading={formData.client_id > 0 ? "Other Projects" : "All Projects"}>
												<CommandItem
													value="none-0"
													onSelect={() => {
												...
												{projects
													?.filter(p => formData.client_id === 0 || p.clientId !== formData.client_id)
													.map(p => (
														<CommandItem
															key={p.id}
															value={`${p.name}-${p.id}`}
